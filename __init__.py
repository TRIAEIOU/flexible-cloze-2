...
def update():
    ...
    msg_box.setText(f"""<div style="text-align: left;">Flexible Cloze update may include both changes in functionality and styling.
    ...
    <li>Select "Cancel" to delay update until next Anki start (for instance, to copy a personalized template to another name).</li></ul></div>""")
    ALL = 0
    ...
    if not (cfront_parts and cback_parts and ccss_parts):
        ans = QMessageBox.critical(mw, 'FCZ update error - overwrite current template?', f'''<div style="text-align: left">Failed to install update, current "Flexible cloze" card template corrupt.
